...
public class UploadSessionController {
    ...
    public ResponseEntity<InitUploadSessionResult> initUploadSession(@Valid @RequestBody InitUploadSessionRequest request, @AuthenticationPrincipal User user ) {
        FileType fileType = fileService.getFileType(request.mediaType.toString());

        InitUploadResult result = uploadSessionService.init(request.filename, request.mediaType, fileType, request.totalSize, request.checksum, user);

        return ResponseEntity.status(HttpStatus.CREATED).body(InitUploadSessionResult.from(result));
    }

    @PostMapping("/uploads/{id}")
    public ResponseEntity<String> writeChunk(@PathVariable UUID id, @Valid @RequestBody MultipartFile chunk, @RequestHeader("Content-Range") String contentRange, @AuthenticationPrincipal User user) {
    ...
    public record InitUploadSessionResult(boolean fileAlreadyExists, String url) {
        public static InitUploadSessionResult from(InitUploadResult result){
